...
- **Example**: `cursor-agent-team/ai_workspace/spec-kit-constitution-B-001.md` → `/full/path/to/project/cursor-agent-team/ai_workspace/spec-kit-constitution-B-001.md`
...
**CRITICAL - ABSOLUTELY MANDATORY**: Before ANY conversion work, AI **MUST execute the command** to retrieve current time.
...
**Version**: v1.0.0-qwen (Qwen Code Adaptation)
**Adapted for**: Qwen Code with strict prompt requirements

**Version History**:
- v1.0.0-qwen (2026-01-15): Qwen Code adaptation with strict time retrieval requirements
- v1.0.0 (2026-01-01): Initial creation - Spec-Kit Translator assistant rules

**Note**: These rules are persistent and automatically applied when using the `/spec_translator` command in Qwen Code. They define the infrastructure and constraints for the Spec-Kit Translator Assistant, while the command itself defines the role behavior and workflow. This version includes strict requirements for Qwen models to prevent time/date hallucinations.
