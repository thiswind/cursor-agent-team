...
### Changed
- **Persona Config Default**: Changed `enabled` default value from `false` to `true`
  - New installations now have persona system enabled by default
  - Users can still disable by setting `enabled: false` in `config/persona_config.yaml`
