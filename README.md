...
Converts execution plans (generated by `/discuss`) into spec-kit formatted documents for spec-driven development workflows. This command automatically transforms Plan files into three spec-kit documents:
...
This command is designed for projects using [spec-kit](https://vibecoding.app/tools/spec-kit) for spec-driven development. For more information about spec-kit, refer to the [spec-kit homepage](https://vibecoding.app/tools/spec-kit).

**Note**: This is an additional feature, not a core team role. The framework will continue to add more additional features in the future.
...
This installs the three core commands and additional features into `.cursor/commands/` and rules into `.cursor/rules/`.
